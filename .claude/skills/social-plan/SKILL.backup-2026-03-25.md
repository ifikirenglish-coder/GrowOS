...
If the brand has posted before, check `brands/[active-brand]/output/social/` for previous plans to infer their usual platforms.
...
Load `brands/[active-brand]/content-ideas.md` if it exists. Scan the "Ready to Write" section for ideas tagged `[TW]`, `[LI]`, `[FB]`, `[IG]`, or `[ALL]`.
...
Also check:
- `brands/[active-brand]/stories/_index.md` — stories that haven't been used recently
- `brands/[active-brand]/audiences/` — pain points and desires to address
...
- Reference relevant stories from `brands/[active-brand]/stories/_index.md` where they fit naturally
...
Save the final plan to: `brands/[active-brand]/output/social/plans/week-YYYY-MM-DD.md`
...
After saving:
- Mark any content bank ideas that were included in the plan as `[SCHEDULED YYYY-MM-DD]` in `brands/[active-brand]/content-ideas.md` (keep them in Ready to Write, just add the tag).
