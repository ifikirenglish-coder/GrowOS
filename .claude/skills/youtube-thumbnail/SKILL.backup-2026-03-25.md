...
**If a youtube-prepare output exists:** Check `brands/[active-brand]/output/youtube/` for recent video scripts. Offer to use that topic.
...
## Step 5 — Generate Thumbnails (if available)

If an image generation tool is available:

1. Generate thumbnail mockups for the top 2-3 variants (or all 5 if the user wants)
2. Use 1280x720 dimensions
3. Ensure text is large and readable at small sizes (mobile search results)
4. Review each output. Regenerate if text is unreadable or composition is off.

---

## Step 6 — Provide Design Briefs (fallback)

If image generation is not available, create Canva/Figma-ready briefs for each variant:
...
Save to: `brands/[active-brand]/output/youtube/YYYY-MM-DD-slug/thumbnails/`
