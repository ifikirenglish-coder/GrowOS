...
1. `brands/[active-brand]/voice.md` — writing voice (for final review)
...
Check `brands/[active-brand]/output/newsletters/drafts/` for recent draft files.
...
1. **Move the draft** from `brands/[active-brand]/output/newsletters/drafts/` to `brands/[active-brand]/output/newsletters/sent/`
2. **Rename the file** to include the send date: `YYYY-MM-DD-[slug].md`
3. **Update sent-index.md** in `brands/[active-brand]/output/newsletters/`. Create the file if it doesn't exist.
